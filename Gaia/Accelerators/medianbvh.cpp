...
bool medianBVH::bounding_box(float t0, float t1, aabb& b) const {
    b = box;
    return true;
}
...
medianBVH::medianBVH(object** l, int n, float time0, float time1) {
    ...
    if (n == 1) {
        ...
        left = new medianBVH(l, n / 2, time0, time1);
        left->type = "bvh_node";
        right = new medianBVH(l + n / 2, n - n / 2, time0, time1);
        right->type = "bvh_node";
    }
